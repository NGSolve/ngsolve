...
cd build/ngsolve
cmake ../../src/ngsolve -DUSE_OCC=ON -DUSE_CCACHE=ON -DUSE_MKL=ON -DUSE_UMFPACK=ON -DINSTALL_PROFILES=ON -DMKL_STATIC=ON -DCMAKE_BUILD_TYPE=Release -DENABLE_UNIT_TESTS=ON -DCPACK_PACKAGING_INSTALL_PREFIX=/opt/netgen $CMAKE_ARGS
make -j12
make install
make package
cd ngsolve
