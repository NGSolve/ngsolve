...
cd build/ngsolve
cmake ../../src/ngsolve -DUSE_NATIVE_ARCH=OFF -DUSE_OCC=ON -DUSE_CCACHE=ON -DUSE_MKL=ON -DUSE_UMFPACK=ON -DINSTALL_PROFILES=OFF -DMKL_STATIC=ON -DENABLE_UNIT_TESTS=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr -DCPACK_PACKAGING_INSTALL_PREFIX=/usr $CMAKE_ARGS
make -j12
make install
make package
cd ngsolve
