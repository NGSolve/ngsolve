...
class ImplicitEuler:
    def __init__(self,
        ...
        self.time = time
        self.gfu_old = ngs.GridFunction(u.space)
        self.dt = ngs.Parameter(dt) if (isinstance(dt, float) or isinstance(dt, int)) else dt
        self.bfmstar = ngs.BilinearForm(u.space)
        self.bfmstar += equation.Replace({ u.dt : 1/self.dt * (u-self.gfu_old) })
        print(equation.Replace({ u.dt : 1/self.dt * (u-self.gfu_old) }))
        self.bfmstar.Assemble()
        self.c = pc_cls(self.bfmstar, **(pc_args or {}))
        self.lin_solver = lin_solver_cls(mat=self.bfmstar, pre=self.c, **(lin_solver_args or {}))
    ...
    def Step(self, u: ngs.GridFunction, dt: Optional[float] = None,
        ...
        self.bfmstar.AssembleLinearization(u.vec)
        newton = ngs.nonlinearsolvers.NewtonSolver(a=self.bfmstar.mat,
                                                   u=u,
