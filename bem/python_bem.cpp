...
void NGS_DLL_HEADER ExportNgsbem(py::module &m)
{
  ...
  m.def("SingleLayerPotentialOperator", [](shared_ptr<FESpace> space, int intorder,
                                           int leafsize, double eta, double eps,
                                           string method) -> shared_ptr<IntegralOperator<>>
  {
    if(!method.compare("dense")) {
       leafsize = INT_MAX;
       eta = 0.;
    }
    BEMParameters param({intorder, leafsize, eta, eps, method});
  ...
  m.def("HelmholtzSingleLayerPotentialOperator", [](shared_ptr<FESpace> trial_space, shared_ptr<FESpace> test_space, double kappa,
                                                    int intorder, int leafsize, double eta, double eps,
                                                    string method) -> shared_ptr<IntegralOperator<Complex>>
  {
    BEMParameters param({intorder, leafsize, eta, eps, method});
                                             ...
                                             string method) -> shared_ptr<IntegralOperator<Complex>>
  {
    ...
    m.def("HelmholtzHypersingularOperator", [](shared_ptr<FESpace> trial_space, shared_ptr<FESpace> test_space, double kappa,  int intorder, int leafsize, double eta, double eps, string method) -> shared_ptr<IntegralOperator<Complex>>
  {
    BEMParameters param({intorder, leafsize, eta, eps, method});
