...
namespace ngbla
{
  ...
  */
  ...
  #ifdef LAPACK
    Array<Complex> work(n*m+100);
    Array<double> rwork(5*min(n,m));
    ngbla::integer lwork = work.Size();

    zgesvd_ ( &jobu, &jobv, &m, &n, A.Data(), &lda,
              S.Data(),
              U.Data(), &ldu, V.Data(), &ldv,
    ...
    for (size_t i = 0; i < H.Width(); i += bs)
      {
        ...
        MultiHouseholderReflectionMem Hv(AsSliceMatrix(Trans(tmp)), memh.Data()); 
        Hv.Mult(M.Rows(rows));
      }
  }
            ...
            if (r > 1e-20 * normmat)
              {
                ...
                zs(i) = r;
                zs(i+1) = 0;
                G.Apply (AsSliceMatrix(Trans(Q)));
                if (i > 0)
                  G.Apply (AsSliceMatrix(Trans(W))); 
              }
            else
              {
                zs(i) = 0;
