...
namespace ngbla
{
  ...
  */
  ...
  public:
    ...
    /// set size and mem
    INLINE FlatVector (size_t as, void * adata)
      : size(as), data(static_cast<TELEM*> (adata)) { }

    /// put FlatVector over fixed size vector
    template <int S>
    INLINE FlatVector (const Vec<S,TSCAL> & v)
    // : size(v.Size()), data(const_cast<T*>(&v(0))) { }
      : size(v.Size()), data(const_cast<T*>(v.Data())) { } 
  ...
  */
  ...
  public:
    ...
    INLINE SliceVector (VectorView<T,TS,TD> v)
      : s(v.Size()), dist(v.Dist()), data(v.Data()) { }
  ...
  public:
    ...
    INLINE auto Shape() const { return tuple(DummySize::Height()); }
  ...
  public:
    ...
    {
      return BareSliceVector<T> (data+i*dist, dist, DummySize(DummySize::Height()-i));
    }
    ...
    {
      return BareSliceVector (data+first*dist, dist*adist, DummySize(DummySize::Height()/adist) );
    }
