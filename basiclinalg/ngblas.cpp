...
#include "matkernel.hpp"
  ...
  */
  ...
  void NgGEMV (double s, BareSliceMatrix<double,RowMajor> a, SliceVector<double> x, SliceVector<double> y,
  ...
  template <typename FUNC>
  void NgGEMV (double s, BareSliceMatrix<double,ColMajor> a, SliceVector<double> x, SliceVector<double> y,
               FUNC func) NETGEN_NOEXCEPT
  {
    // y.Range(a.Height()) = s*a*x;
    for (size_t i = 0; i < y.Size(); i++)
  ...
  */
  ...
  template <>
  NGS_DLL_HEADER    
  void NgGEMV<false> (double s, BareSliceMatrix<double,RowMajor> a, FlatVector<const double> x, FlatVector<double> y) NETGEN_NOEXCEPT
  {
    T_NgGEMV (s, a, x, y, [](double & y, double sum) { y=sum; });
  }
  template <>
  NGS_DLL_HEADER      
  void NgGEMV<true> (double s, BareSliceMatrix<double,RowMajor> a, FlatVector<const double> x, FlatVector<double> y) NETGEN_NOEXCEPT
  {
    T_NgGEMV (s, a, x, y, [](double & y, double sum) { y+=sum; });
  }
  template <>
  NGS_DLL_HEADER      
  void NgGEMV<false> (double s, BareSliceMatrix<double,ColMajor> a, FlatVector<const double> x, FlatVector<double> y) NETGEN_NOEXCEPT
  {
    T_NgGEMV (s, a, x, y, [](double & y, double sum) { y=sum; });
  }
  template <>
  NGS_DLL_HEADER      
  void NgGEMV<true> (double s, BareSliceMatrix<double,ColMajor> a, FlatVector<const double> x, FlatVector<double> y) NETGEN_NOEXCEPT
  {
    T_NgGEMV (s, a, x, y, [](double & y, double sum) { y+=sum; });
  }
  ...
  template <typename TM, typename FUNC>
  void T_NgGEMV (double s, BareSliceMatrix<TM,RowMajor> a, SliceVector<double> x, SliceVector<double> y, FUNC func) NETGEN_NOEXCEPT  
  {
    for (size_t i = 0; i < y.Size(); i++)
      {
        double sum = 0;
  ...
  template <typename TM, typename FUNC>
  void T_NgGEMV (double s, BareSliceMatrix<TM,ColMajor> a, SliceVector<double> x, SliceVector<double> y, FUNC func) NETGEN_NOEXCEPT  
  {
    for (size_t i = 0; i < y.Size(); i++)
      {
        double sum = 0;
  ...
  */
  ...
  void AddABt (SliceMatrix<SIMD<double>> a,
               ...
               SliceMatrix<Complex> c)
  {
    // RegionTimer reg(timer_addabtdc);
  ...
  void AddABt (SliceMatrix<double> a, SliceMatrix<Complex> b, BareSliceMatrix<Complex> bc)
  {
    // c.AddSize(a.Height(), b.Height()) += a * Trans(b);
    // return;
  ...
  void AddABtSym (SliceMatrix<double> a, SliceMatrix<Complex> b, BareSliceMatrix<Complex> bc)
  {
...
{
  constexpr int SW = SIMD<double>::Size();
  SIMD<double> conj([](int i) { return (i%2==0) ? 1. : -1.; });
  SIMD<double> sum0_0(0);
  SIMD<double> sum0_1(0);
  SIMD<double> sum1_0(0);
  SIMD<double> sum1_1(0);
  SIMD<double> sum2_0(0);
  SIMD<double> sum2_1(0);
  double* pa0 = (double*) ppa[0];
  double* pa1 = (double*) ppa[1];
  double* pa2 = (double*) ppa[2];
  double* pb0 = (double*) ppb[0];
  double* pb1 = (double*) ppb[1];
  size_t i = 0;
  for (; i+SW <= 2*n; i+=SW) {
    SIMD<double> a0(pa0 + i);
    auto [a0Re, a0Im] = Unpack(a0, a0);
    SIMD<double> a1(pa1 + i);
    auto [a1Re, a1Im] = Unpack(a1, a1);
    SIMD<double> a2(pa2 + i);
    auto [a2Re, a2Im] = Unpack(a2, a2);
    SIMD<double> b0 (pb0 + i);
    SIMD<double> b0Swap = SwapPairs(b0);
    SIMD<double> a0Im_b0Swap = a0Im * b0Swap;
    SIMD<double> a1Im_b0Swap = a1Im * b0Swap;
    SIMD<double> a2Im_b0Swap = a2Im * b0Swap;
    sum0_0 += FMAddSub (a0Re, b0, a0Im_b0Swap);
    sum1_0 += FMAddSub (a1Re, b0, a1Im_b0Swap);
    sum2_0 += FMAddSub (a2Re, b0, a2Im_b0Swap);
    SIMD<double> b1 (pb1 + i);
    SIMD<double> b1Swap = SwapPairs(b1);
    SIMD<double> a0Im_b1Swap = a0Im * b1Swap;
    SIMD<double> a1Im_b1Swap = a1Im * b1Swap;
    SIMD<double> a2Im_b1Swap = a2Im * b1Swap;
    sum0_1 += FMAddSub (a0Re, b1, a0Im_b1Swap);
    sum1_1 += FMAddSub (a1Re, b1, a1Im_b1Swap);
    sum2_1 += FMAddSub (a2Re, b1, a2Im_b1Swap);
  }
  int r = (2*n) % SW;
  if (r) {
    SIMD<mask64> mask(r);
    SIMD<double> a0(pa0 + i, mask);
    auto [a0Re, a0Im] = Unpack(a0, a0);
    SIMD<double> a1(pa1 + i, mask);
    auto [a1Re, a1Im] = Unpack(a1, a1);
    SIMD<double> a2(pa2 + i, mask);
    auto [a2Re, a2Im] = Unpack(a2, a2);
  ...
  { SIMD<double,2> hsum = HSum128(sum0_0);
    pc[0+0*dc].real(hsum[0]);
    pc[0+0*dc].imag(hsum[1]); }
  { SIMD<double,2> hsum = HSum128(sum0_1);
    pc[1+0*dc].real(hsum[0]);
    pc[1+0*dc].imag(hsum[1]); }
  { SIMD<double,2> hsum = HSum128(sum1_0);
    pc[0+1*dc].real(hsum[0]);
    pc[0+1*dc].imag(hsum[1]); }
  { SIMD<double,2> hsum = HSum128(sum1_1);
    pc[1+1*dc].real(hsum[0]);
    pc[1+1*dc].imag(hsum[1]); }
  { SIMD<double,2> hsum = HSum128(sum2_0);
    pc[0+2*dc].real(hsum[0]);
    pc[0+2*dc].imag(hsum[1]); }
  { SIMD<double,2> hsum = HSum128(sum2_1);
    pc[1+2*dc].real(hsum[0]);
    pc[1+2*dc].imag(hsum[1]); }
}
void TestFunc31
(size_t n,
 Complex** ppa,
...
void T_AddABt (SliceMatrix<Complex> a, SliceMatrix<Complex> b, BareSliceMatrix<Complex> c,
               FUNC func)
  {
    // c.AddSize(a.Height(), b.Height()) += a * Trans(b); // | Lapack;
    // return ;
    Matrix<Complex> block(6,2);
    size_t i = 0;
    for ( ; i+6 <= a.Height(); i+=6)
      {
        size_t j = 0;
        for ( ; j+2 <= b.Height(); j+=2)
          {
            Complex *pa[6];
            Complex *pb[2];
            pa[0] = &a(i,0);
