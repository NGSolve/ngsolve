...
namespace ngcomp
{
  ...
  void HCurlAMG_Matrix<SCAL> :: Init (shared_ptr<SparseMatrixTM<SCAL>> _mat,
                                 ...
                                 size_t level)
  {
    mat = _mat; size = mat->Height();
    static Timer timer("HCurlAMG_Matrix");
    ...
    AMGInfo cinfo;
    Table<int> ce2f;
    shared_ptr<Table<int>> smoothing_blocks;
    for(auto coarsening : Range(coarsenings_per_level))
      {
        if(coarsening == 0)
          {
            auto coll_weights = CalcEdgeCollapse(f2e, e2v, edge_weights,
                                                 face_weights, e2f);
...
//             // build smoother
            if(blockjacobi_smoother)
              {
                ...
                smoothing_blocks = make_shared<Table<int>>(smoothing_blocks_creator.MoveTable());
          ...
          {
            auto coll_weights = CalcEdgeCollapse(cinfo.f2e, cinfo.e2v,
                                                 cinfo.edge_weights,
  ...
  template<typename SCAL>
  Array<double> HCurlAMG_Matrix<SCAL> :: CalcEdgeCollapse
