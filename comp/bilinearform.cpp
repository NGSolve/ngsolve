...
namespace ngcomp
{
  ...
  */
    ...
    try
      {
        ...
        auto src_file = filesystem::path(prefix).concat(".cpp").u8string();
        auto obj_file = filesystem::path(prefix).concat(".o").u8string();
        ...
        int err = system( ("ngscxx -c "+src_file+" -o "+obj_file).c_str() );
  ...
  template <class SCAL>
  void S_BilinearForm<SCAL> :: Assemble_facetwise_skeleton_parts_BND (Array<bool>& useddof, size_t & gcnt, LocalHeap & clh, const BaseVector * lin)
  {
    int ne = ma->GetNE(BND);
    gcnt += ne;
		...
		*testout << " hasbbnd = " << (VB_parts[BBND].Size()>0) << endl;
	    ...
	    // for (VorB vb : { VOL, BND })
            ...
            */
            ...
            if (facetwise_skeleton_parts[BND].Size())
              Assemble_facetwise_skeleton_parts_BND(useddof,gcnt,clh);
    ...
    try
      {
        ...
        if (facetwise_skeleton_parts[BND].Size()){
          size_t dummy;
          Assemble_facetwise_skeleton_parts_BND(useddof, dummy, clh, &lin);
        }
