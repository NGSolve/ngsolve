...
namespace ngcomp
{
  ...
  **/
  ...
  void HypreAMSPreconditioner :: Setup ()
  {
    ...
    MPI_Comm comm = this->parallel ? MPI_Comm(this->hcurlfes->GetParallelDofs()->GetCommunicator()) : MPI_COMM_WORLD;
    this->rank = MyMPI_GetId(comm);
    this->np = MyMPI_GetNTasks(comm);
    this->parallel = (np>1);
