...
namespace ngcomp
{
  ...
  public:
    ...
    {
      return els_on_level[level] * (order+1)*(order+2)/2;
    }
    ...
    virtual shared_ptr<SparseMatrix< double >> CreateProlongationMatrix( int finelevel ) const override
    {
      ...
      for(auto i : Range(fine_els))
        {
          ...
          while(parent >= coarse_els)
            {
              parent = ma->GetParentElement(ElementId(vb, parent)).Nr();
              if(parent != -1)
                max_parent[i] = parent;
            }
        }
      ...
      for(auto i : Range(coarse_els, fine_els))
        {
          int child = i;
          int parent = ma->GetParentElement (ElementId(vb,child)).Nr();
          ...
          auto range_parent = Range(ndel * parent,
                                    ndel * (parent+1));
          int classnr = trig_creation_class[child];
          matmul(trigprolsR[classnr], trigprolsL[classnr],
  ...
  class L2HoProlongationTet : public Prolongation
  {
    ///
    shared_ptr<MeshAccess> ma;
    ///
      ...
      for (int classnr = 0; classnr < 1024; classnr++)
        {
          ...
          size_t vertsfL[5] = { verts[4], verts[1], verts[2], verts[3] };
          size_t vertsfR[5] = { verts[0], verts[1], verts[2], verts[4] };
          ...
          if(parent!=-1)
            {
              ...
              tmp = fv.Range(ndel*parent, ndel*(parent+1));
              fv.Range(ndel*i, ndel*(i+1)) = tetprolsR[classnr] * tmp;
              fv.Range(ndel*parent, ndel*(parent+1)) = tetprolsL[classnr] * tmp;
            }
        }
    }
      ...
      for (size_t i = ne-1; i >= nec; i--)
        {
          ...
          if(parent!=-1)
            {
              int classnr = tet_creation_class[i];
              tmp = Trans (tetprolsR[classnr]) * fv.Range(ndel*i, ndel*(i+1)) +
                Trans (tetprolsL[classnr]) * fv.Range(ndel*parent, ndel*(parent+1));
              fv.Range(ndel*parent, ndel*(parent+1)) = tmp;
            }
        }
    }
...
/*
  ...
  class VectorL2HoProlongationTrig : public Prolongation
  {
    ///
    shared_ptr<MeshAccess> ma;
    ///
  ...
  public:
    ...
    {
        return els_on_level[level] * (order+1)*(order+2)/2 * ma->GetDimension();
    }
          ...
          if(parent!=-1)
            {
              ...
              tmp2 = trigprolsR[classnr] * tmp1;
              fvm.Cols(i*ndel, (i+1)*ndel) = tmp2.AsMatrix(dim, ndel);

              tmp2 = trigprolsL[classnr] * tmp1;
          ...
          if(parent!=-1)
            {
              ...
              tmp = Trans(trigprolsR[classnr]) * tmpR +
                    Trans (trigprolsL[classnr]) * tmpL;
