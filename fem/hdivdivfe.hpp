...
namespace ngfem
{
  ...
  public:
	...
	for (size_t i = 0; i < mir.Size(); i++)
	{
          ...
          TIP<DIM,AutoDiffDiff<DIM,SIMD<double>>> addp(adp);
	  Cast() -> T_CalcShape
            (addp, SBLambda([divshapes,i,trans](int j,auto val)
                            {
        ...
        for (size_t i = 0; i < mir.Size(); i++)
          {
            ...
            // mir[i].CalcHesse (hesse);

            mir.GetTransformation().CalcHesse (mir.IR()[i], hesse);
            /*
            diff = hesse-hesse2;
            if (HSum(L2Norm2(diff)) > 1e-16)
              cout << "hesse = " << hesse << endl
                   << "hesse2 = " << hesse2 << endl;
            */
...
#define HDIVDIVFE_EXTERN extern
  ...
  public:
   ...
   template <typename Thx, typename TFA> 
    void T_CalcShape (TIP<2,Thx> ip, TFA & shape) const
    {
