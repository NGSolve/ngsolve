...
namespace ngfem
{
  ...
  shared_ptr<CoefficientFunction> MakeCoefficient (py::object val)
  {
    ...
    try { return MakeConstantCoefficientFunction(val.cast<Complex>()); }
    catch(const py::cast_error&) {}
...
val : can be one of the following:
  ...
  list of scalars or CoefficientFunctions:
    ...
    .def_property_readonly("shape",
                           [] (CF& self) { return makePyTuple(self.Dimensions()); },
                           "shape of CF")
...
//           cout << "comps: " << comps << endl;
    ...
    {return coef + MakeConstantCoefficientFunction(val); }, py::arg("value"))
...
cf : ngsolve.CoefficientFunction
  input CoefficientFunction
    ...
    .def ("Diff", // &CoefficientFunction::Diff,
          ...
          {
            if (!var->IsVariable())
              cout << IM(1) << "Warning: differentiationg by a variable not marked as Variable, \n"
    ...
    .def ("__rtruediv__", [] (shared_ptr<CF> coef, Complex val)
          { return MakeConstantCoefficientFunction(val) / coef; }, py::arg("value"))
