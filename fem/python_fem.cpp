    ...
    .def("__call__", [](shared_ptr<CF> self, py::array_t<MeshPoint> points) -> py::array
         {
           ...
           auto pts = points.unchecked<1>(); // pts has array access without bound checks
           size_t npoints = pts.shape(0);
           auto func = [&] (auto& array, auto&& range, LocalHeap& lh)
             {
               for(auto i : range)
                 {
                   HeapReset hr(lh);
                   ...
                   FlatVector<typename std::remove_reference<decltype(array[0])>::type> fv(self->Dimension(), &array[i*self->Dimension()]);
           ...
           if (!self->IsComplex())
             {
               ...
               if(task_manager)
                 {
                   SharedLoop sl(npoints);
                   task_manager->CreateJob([&](const TaskInfo& ti)
                                           {
                                             LocalHeap tlh = lh.Split(ti.thread_nr, ti.nthreads);
                                             func(vals, sl, tlh);
                                           });
                 }
               else
                 {
                   func(vals, Range(npoints), lh);
                 }
               auto np_array = MoveToNumpyArray(vals);
...
weights : list
  ...
  static_assert(sizeof(MeshPoint) == 5*sizeof(double));
  static_assert(sizeof(VorB) == sizeof(int));
