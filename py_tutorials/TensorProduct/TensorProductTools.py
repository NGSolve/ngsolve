...
def MakeMesh2D(mesh1,mesh2):
    ...
    for elx in els1:
        for ely in els2:
            ...
            # pnum = [(elx.vertices[0].nr-1) * len(vert2) + ely.vertices[0].nr-1,
                    # (elx.vertices[1].nr-1) * len(vert2) + ely.vertices[0].nr-1,
...
def MakeMesh3D(mesh1,mesh2):
    # Note: First mesh is 2d, second mesh is 1d!!
    tpmesh = ngmeshing.Mesh(dim = mesh1.dim + mesh2.dim)
    ngm1 = mesh1.ngmesh
    ngm2 = mesh2.ngmesh
...
def AddSurfElements2D(tpmesh,mesh1,mesh2):
    ...
    tpmesh.Add (FaceDescriptor(surfnr=2,domin=1,bc=2))
    for elx in els1:
        vert_loc = elx.vertices
        vert_glob = []
        for vx in vert_loc:
            vert_glob.append(PointId((vx.nr-1)*len(ngm2.Points())+1))
        tpmesh.Add(Element2D(2,vert_glob))
    els1 = ngm1.Elements1D()
    for elx in els1:
        for ely in els2:
            vert_glob=[]
...
#                for vx in elx.vertices:
            ...
            #tpmesh.Add(Element2D(2,[PointId(6),PointId(7),PointId(2),PointId(1)]))
            tpmesh.Add(Element2D(2,vert_glob))
    return tpmesh